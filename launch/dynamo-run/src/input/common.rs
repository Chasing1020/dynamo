        ...
        EngineConfig::Dynamic(endpoint_id) => {
            ...
            let engine: OpenAIChatCompletionsStreamingEngine = match &flags.router_mode {
                RouterMode::Random | RouterMode::RoundRobin => {
                    ...
                    let mut card = network_name.load_mdc(endpoint_id, etcd_client).await?;

                    // TODO We need to check ModelEntry.model_type to decide if we have a chat or
                    // completions model, and adjust the generics (NvCreateChatCompletionRequest)
