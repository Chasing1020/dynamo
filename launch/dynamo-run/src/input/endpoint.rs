    ...
    let (rt_fut, mut card) = match engine_config {
        ...
        EngineConfig::StaticCore {
            ...
            let endpoint = register(
                ...
                endpoint_id,
                *card.clone(),
                // Ingress is handling pre-processing so we don't mind which model type
                ModelType::all(),
            )
            .await?;
...
async fn register(
    ...
    if let Some(etcd_client) = distributed_runtime.etcd_client() {
        ...
        for model_type in model_types.into_iter() {
            ...
            etcd_client
                .kv_create(
                    // We watch the prefix so adding model_type on end is fine
                    // TODO We need to figure out the names and move it into dynamo-llm.
                    format!("{network_name}.{model_type}"),
